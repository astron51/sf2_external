...
bool Direct::D3DInit(HWND hwnd,Direct& result)
{
	...
	if (FAILED(D3DXCreateFontA(res.dx_Device, 16, 0, 400, 1, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, "Microsoft Yahei", &res.dx_font))) {
		return false;
	}
...
void Direct::DrawStringWCH(int x, int y, int a, int r, int g, int b, wchar_t text[])
{
	D3DCOLOR col = D3DCOLOR_ARGB(a, r, g, b);
	RECT rect;
	rect.left = x;
	...
	rect.bottom = y + 30;
	dx_font->DrawTextW(NULL, text, -1, &rect, DT_LEFT | DT_WORDBREAK, col);
}
