...
BOOL CALLBACK EnumWindowsCallback(HWND hwnd, LPARAM lParam) {
	...
	if (wcscmp(L"OdinSF", className) == 0) {
	...
	while (running) {
		...
		if (CurrentDirectX.dx_Device != 0) {
			try {
				...
				CurrentDirectX.DrawString(10, 300, 255, rs, gs, bs, title);
				
				if ((CurrentTickCount - LastTickCount) > 1.0f)
				{
					LastTickCount = CurrentTickCount;
					curFrame = std::to_wstring(Fps); curFrame = L"FPS: " + curFrame.substr(0, curFrame.find('.'));
					Fps = 0;
				}
				CurrentDirectX.DrawString(90, 300, 255, 46, 135, 190, curFrame.c_str());
				Render();
